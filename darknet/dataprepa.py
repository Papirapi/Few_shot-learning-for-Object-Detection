...
train_file_list = [path.split('/')[-1].split('.')[0]+'.xml' for path in dataset_file.readlines()]
...
def create_detection_distributor(train_set=True):
    ...
    for i in range(len(file_list)):
        img_path = os.path.join('/dataset/VOCtrainval12/VOCdevkit/VOC2012/JPEGImages/', data_set[i][0])
