...
class yolo_v2(object):
    def __init__(self, isTraining = True):
        ...
        # self.offset = tf.reshape(tf.constant(self.offset, dtype=tf.float32), [1,self.num_class, self.cell_size, self.cell_size, self.box_per_cell])
        # self.offset = tf.tile(self.offset, (self.batch_size, 1,1, 1, 1))
    ...
    def conv_layer(self, inputs, shape, batch_norm = True, name = '0_conv', padding='SAME'):
        weight = tf.Variable(tf.truncated_normal(shape, stddev=0.1), name='weight')
        biases = tf.Variable(tf.constant(0.1, shape=[shape[4]]), name='biases')

        conv = tf.nn.conv3d(inputs, weight, strides=[1, 1, 1, 1, 1], padding=padding, name=name)
    ...
    #     predict = tf.reshape(predict, [self.batch_size,self.num_class, self.cell_size, self.cell_size, self.box_per_cell, self.num_class + 5])
    #     box_coordinate = tf.reshape(predict[..., :4], [self.batch_size,self.num_class,self.cell_size, self.cell_size, self.box_per_cell, 4])
    #     box_confidence = tf.reshape(predict[..., 4], [self.batch_size,self.num_class,self.cell_size, self.cell_size, self.box_per_cell, 1])
    #     box_classes = tf.reshape(predict[..., 5:], [self.batch_size,self.num_class, self.cell_size, self.cell_size, self.box_per_cell, self.num_class])
    ...
    #     box_coor_trans = tf.transpose(boxes1, (1, 2, 3, 4, 5, 0))
    #     box_confidence = 1.0 / (1.0 + tf.exp(-1.0 * box_confidence))
    #     box_classes = tf.nn.softmax(box_classes)
        
    #     response = tf.reshape(label[..., 0], [self.batch_size,self.num_class, self.cell_size, self.cell_size, self.box_per_cell])
    #     boxes = tf.reshape(label[..., 1:5], [self.batch_size,self.num_class, self.cell_size, self.cell_size, self.box_per_cell, 4])
    #     classes = tf.reshape(label[..., 5:], [self.batch_size,self.num_class, self.cell_size, self.cell_size, self.box_per_cell, self.num_class])
    ...
    def calc_iou(self, boxes1, boxes2):
        ...
        box = tf.stack([boxes1[..., 0] - boxx[..., 0] * 0.5,
                        ...
                        boxes1[..., 0] + boxx[..., 0] * 0.5,
                        boxes1[..., 1] + boxx[..., 1] * 0.5])
        boxes1 = tf.transpose(box, (1, 2, 3, 4, 5, 0))
        ...
        box = tf.stack([boxes2[..., 0] - boxx[..., 0] * 0.5,
                        ...
                        boxes2[..., 0] + boxx[..., 0] * 0.5,
                        boxes2[..., 1] + boxx[..., 1] * 0.5])
        boxes2 = tf.transpose(box, (1, 2, 3, 4, 5, 0))
